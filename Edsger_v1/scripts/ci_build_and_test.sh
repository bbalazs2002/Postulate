...
# docker run below) turned out to be unrelated: Edsger_v1/Dockerfile
...
docker build "${BUILD_FLAGS[@]}" -f Edsger_v1/Dockerfile -t postulate-edsger-v1 Edsger_v1
docker inspect postulate-edsger-v1 --format 'postulate-edsger-v1: Os={{.Os}} Architecture={{.Architecture}}'

# Runs Edsger_v1/scripts/build.sh's own single-stage build (real src/
# modular/ source, compiled by the vendored codegen_selfhost binary)
# inside its builder stage; see Edsger_v1/Dockerfile.release's own
# header comment.
docker build "${BUILD_FLAGS[@]}" -f Edsger_v1/Dockerfile.release -t postulate-edsger-v1-release Edsger_v1
docker inspect postulate-edsger-v1-release --format 'postulate-edsger-v1-release: Os={{.Os}} Architecture={{.Architecture}}'

# codegen_cases fixtures, run directly against the real, released
# build/codegen -- baked into postulate-edsger-v1-release at
# /opt/edsger/build/codegen (not the historical single-file
# src/codegen.ptl that Edsger_v1/tests/run_all_tests.sh's own
# run_codegen_tests.sh checks). Deliberately NOT run against
# postulate-edsger-v1 (the dev image): that image never bakes in Edsger's
# own source or a built binary (see its own header comment) -- only
# postulate-edsger-v1-release's own build stage actually produces
...
docker run --rm -v "$PWD/Edsger_v1/tests:/tests" --entrypoint bash postulate-edsger-v1-release -c "
  set -e
