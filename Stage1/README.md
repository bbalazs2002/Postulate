...
| Codegen | `src/codegen.ptl` | Done, `#include` included, on the `v1.0.2` LLVM IR backend (`docs/postulate_stage1_v1_0_2_llvm_backend_design.md`), and now **all of `v1.0.3`** (`docs/postulate_stage1_v1_0_3_composites_design.md`) — struct-typed locals, field read/write (nested chains included), struct literals (nested included), struct-to-struct copy; array-typed locals, index read/write (chained index+field lvalues like `pts[1].x` included), array literals (array-of-struct included), broadcast-init (scalar *or* struct source, both at decl-init and at assignment), array-to-array copy; struct/array-typed **parameters and return values, passed/returned by value with full independent-copy semantics** (`type_ok`'s scalar-only restriction removed for these, `llc`'s own ABI lowering does the actual register-vs-memory work); `&`/`*`, `null`, pointer-typed locals/parameters/return values, `==`/`!=` pointer comparison, for **any pointee** — scalar (`*int32`), struct (`*Point`), or array (`*int32[3]`/`*(int32[3])`), including a self-referential struct field (`next: *Node`, the linked-list shape) — via LLVM's native packed struct types (`%struct.Name = type <{ ... }>`, zero padding, matching v0 §2.6 exactly), native array types (`[N x T]`), opaque `ptr` for pointers, and a real `gen_lvalue`. The one deliberately out-of-scope case left in all of `v1.0.3` is `**T` (pointer-to-pointer); direct field/index access on a bare call result or literal with no intermediate variable (`f().field`) also still doesn't work — a pre-existing property of `gen_lvalue`'s own lvalue-only recursion. Also carries `v1.0.2`'s one deliberate, documented exception: function-call argument evaluation order is **left to right** (v1's rule), not v0's right-to-left. Verified end-to-end (lowered with `llc -filetype=obj`, linked with `ld`, actually **run**) against every scalar-only-compatible fixture in `Hoare/tests/codegen_cases/*` (`01`–`10`, `19`, `20`), the 16 `include_cases` fixtures, and `Stage1/tests/codegen_cases/*` (eval-order, narrow-type wraparound, struct field/literal/copy-isolation, array index/literal/broadcast-init/copy-isolation, struct-source/assignment-time broadcast-init with its own copy-isolation and mismatch-rejection checks, pointer round-trip/null-comparison/param-and-return-value tests, struct/array parameters+return values with copy-isolation and call-chaining, and composite-pointee round-trips including a linked-list walk — see each fixture's own header comment). |
