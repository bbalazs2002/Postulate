...
| Codegen | `src/codegen.ptl` | Done, `#include` included, on the `v1.0.2` LLVM IR backend (`docs/postulate_stage1_v1_0_2_llvm_backend_design.md`), through `v1.0.3`'s width-unification sub-pass, and now also `v1.0.3`'s struct **and array** sub-passes (`docs/postulate_stage1_v1_0_3_composites_design.md`) — struct-typed locals, field read/write (nested chains included), struct literals (nested included), struct-to-struct copy; array-typed locals, index read/write (chained index+field lvalues like `pts[1].x` included), array literals (array-of-struct included), broadcast-init (scalar *or* struct source, both at decl-init and at assignment), array-to-array copy — via LLVM's native packed struct types (`%struct.Name = type <{ ... }>`, zero padding, matching v0 §2.6 exactly), native array types (`[N x T]`), and a real `gen_lvalue`. Struct/array-typed **parameters and return values, and pointers (`&`/`*`), are still deliberately out of scope** (rejected with `codegen error`). Also carries `v1.0.2`'s one deliberate, documented exception: function-call argument evaluation order is **left to right** (v1's rule), not v0's right-to-left. Verified end-to-end (lowered with `llc -filetype=obj`, linked with `ld`, actually **run**) against every scalar-only-compatible fixture in `Hoare/tests/codegen_cases/*` (`01`–`10`, `19`, `20`), the 16 `include_cases` fixtures, and `Stage1/tests/codegen_cases/*` (eval-order, narrow-type wraparound, struct field/literal/copy-isolation, array index/literal/broadcast-init/copy-isolation, and struct-source/assignment-time broadcast-init with its own copy-isolation and mismatch-rejection checks — see each fixture's own header comment). |
