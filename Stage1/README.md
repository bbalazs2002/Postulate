...
| Codegen | `src/codegen.ptl` | Done, `#include` included, on the `v1.0.2` LLVM IR backend (`docs/postulate_stage1_v1_0_2_llvm_backend_design.md`), and now also through `v1.0.3`'s first sub-pass, width unification (`docs/postulate_stage1_v1_0_3_composites_design.md`) — every scalar value (not just future struct fields) uses its true declared LLVM width (i1/i8/i16/i32/i64) instead of `v1.0.2`'s uniform `i64`, fixing narrow-type arithmetic wraparound deliberately as part of the change. Composites (`v1.0.3`'s second sub-pass — struct/array types, literals, field/index access, the calling convention) are **not yet implemented**; still scalar-only, still cleanly rejected with `codegen error` for structs/arrays/pointers. Also carries `v1.0.2`'s one deliberate, documented exception: function-call argument evaluation order is **left to right** (v1's rule), not v0's right-to-left. Verified end-to-end (lowered with `llc -filetype=obj`, linked with `ld`, actually **run**) against every scalar-only-compatible fixture in `Hoare/tests/codegen_cases/*` (`01`–`10`, `19`, `20`, all producing identical exit codes to before this sub-pass), the 16 `include_cases` fixtures, and `Stage1/tests/codegen_cases/*` (the `v1.0.2` eval-order fixture plus a new one proving narrow-type wraparound now works — see that fixture's own header comment). |
