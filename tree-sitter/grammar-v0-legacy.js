// Postulate v0 grammar -- kept for reference only, 2026-08-29, when the
// active grammar.js was rewritten to cover v1.0 (what Edsger_v0 actually
// accepts, docs/postulate_v1_0_language_reference.md) instead. NOT built
// or shipped by anything: `tree-sitter generate` only ever reads
// `grammar.js`, never this file. To revive v0 highlighting, copy this
// over `grammar.js`, run `tree-sitter generate`, then `tree-sitter build
// --wasm`, and copy the result over `../editors/vscode/tree-sitter-
// postulate.wasm` (and update `queries/highlights.scm` back down to v0's
// own, smaller keyword/node set -- the current one assumes v1.0 nodes
// like `namespace_decl`/`use_decl`/`char_literal` that don't exist here).
//
// This is the exact grammar Hoare (Stage 0) compiles today -- no
// namespace/use/@autoload, no char/as/pointer-arithmetic/sizeof/
// lengthof/uintptr, no string literals anywhere.
